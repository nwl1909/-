// ============================================================
// Дурак Online — настройки подключения к Supabase
// ============================================================
// Впишите сюда данные вашего проекта с supabase.com:
// Project Settings → API → Project URL и Project API keys → anon public
//
// Это публичный (anon) ключ — его можно спокойно хранить в коде на
// GitHub, он не даёт полного доступа к базе, только то, что разрешено
// политиками RLS (см. supabase_setup.sql).
// ============================================================

const SUPABASE_URL = "https://aakqnfzaoimpvbdlwuyr.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFha3FuZnphb2ltcHZiZGx3dXlyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQ0MDUzMDYsImV4cCI6MjA5OTk4MTMwNn0.shGFhLAGxjs_QJd3Bbp5aib_lm8SRsWBBhm8AM1HaS8";
